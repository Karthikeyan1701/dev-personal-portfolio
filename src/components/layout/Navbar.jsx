...
const Navbar = () => {
  ...
  return (
    <nav
        className={`fixed top-0 left-0 right-0 z-1000 w-full py-4 transtion-all duration-300 ${isScrolled
            ...
            <div className='flex items-center justify-between'>
                ...
                <div className='flex items-center gap-4'>
                    ...
                    <button
                      ...
                      className='text-2xl font-bold bg-linear-to-r from-primary via-primary to-primary/30 bg-clip-text text-transparent hover:opacity-80 transition-opacity'
                      aria-label='home'
                    >
                      {PERSONAL_INFO.name
                        .trim()
                ...
                <div className='bg-black/95 backdrop-blur-lg border-t border-white/10 px-5 py-6 space-y-3'>
                    {NAV_LINKS.map((link) => (
                        <button
                            key={link.id}
                            onClick={() => handleNavClick(link.id)}
                            className={`block w-full text-left px-4 py-3 rounded-lg font-medium transition-all ${activeSection === link.id
